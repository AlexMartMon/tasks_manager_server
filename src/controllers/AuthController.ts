...
export class AuthController {
    ...
    static postCreateAccount = async (req: Request, res: Response) => {
        try {
            ...
            if (userExist) {
                const error = new Error('Current email already registed, try again with another email.')
            ...
            res.status(201).send('Account created succesfully! check your email to confirm your account.')
        } catch (error) {
    ...
    static postLogin = async (req: Request, res: Response) => {
        try {
            ...
            if (!user.confirmed) {
                ...
                const error = new Error("User email not confirmed yet. We've sent you a email to validateit, check your emails.")
